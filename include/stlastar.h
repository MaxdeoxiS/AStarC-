...
public: // methods
	...
	AStarSearch() : m_State( SEARCH_STATE_NOT_INITIALISED ),m_CurrentSolutionNode( NULL ),m_AllocateNodeCount(0),m_CancelRequest( false ){}
	AStarSearch(int MaxNodes) : m_State( SEARCH_STATE_NOT_INITIALISED ),m_CurrentSolutionNode( NULL ),m_AllocateNodeCount(0),m_CancelRequest( false ){}
	...
	void SetStartAndGoalStates( UserState &Start, UserState &Goal )
	{
		...
		std::cout << m_Goal->m_UserState.x << m_Goal->m_UserState.y << std::endl;

		m_State = SEARCH_STATE_SEARCHING;

		// Initialise the AStar specific parts of the Start Node; The user only needs fill out the state information
		m_Start->g = 0;
	...
	unsigned int SearchStep()
	{
		...
		// Failure is defined as emptying the open list as there is nothing left to search...Allow user abort
		...
		else // not goal
		{
			...
			for( typename vector< Node * >::iterator successor = m_Successors.begin(); successor != m_Successors.end(); successor ++ )
			{
				...
				// If it is but the node that is already on them is better (lower g) then we can forget about this successor
	...
	// Get final cost of solution, Returns FLT_MAX if goal is not defined or there is no solution
	...
	// For educational use and debugging it is useful to be able to view the open and closed list at each step, here are two functions to allow that.
